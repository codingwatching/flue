  ...
  async __flueRetryAgentSubmissions(_payload, schedule) {
    if (!(await reconcileFlueAgentSubmissions(this, "assistant"))) {
      await this.cancelSchedule(schedule.id);
		...
		expect(entry).toContain('if (attemptMarkers.keys.has(submissionAttemptMarkerKey(submission)) && submission.recoveryRequestedAt === undefined) continue;');
		expect(entry).toContain('await reconcileInterruptedSqlAgentSubmission(submission, doInstance, agentName);');
		expect(entry).toContain('const submission = submissions.requestSubmissionRecovery(submissionId, attemptId);');
		...
		expect(entry).toContain("running = doInstance.runFiber('flue:dispatch-attempt', async (fiberCtx) => {");
