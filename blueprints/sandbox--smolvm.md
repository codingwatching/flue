...
# Add a Flue Sandbox Adapter: smolvm

You are an AI coding agent installing the [smolvm](https://smolmachines.com) sandbox
adapter for a Flue project. Follow these instructions exactly. Confirm with the
...
this adapter just adapts the running machine so Flue agents can run shell
...
hypervisor is required, this adapter only works on macOS or Linux hosts and
...
This adapter imports from `smolvm-embedded`, so the user's project needs to
...
automatically. The smolvm CLI is **not** required for this adapter.
...
The adapter adapts an existing machine. Create or connect to one with the
...
The adapter **never** calls `delete()` on the user's behalf.
