 ...
 */
async function createDefaultEnv(scope, workspaceStore) {
...
function createContextForRequest(agentName, id, runId, payload, doInstance, req) {
  // Use DO SQLite storage by default, fall back to in-memory
  const sql = doInstance?.ctx?.storage?.sql;
  const defaultWorkspaceStore = sql
    ? createDurableDefaultWorkspaceStore(sql)
